...
impl fmt::Display for CPU {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(f, "A: {:#04x} F: {:#04x} B: {:#04x} C: {:#04x} D: {:#04x} E: {:#04x} H: {:#04x} L: {:#04x} SP: {:#04x} PC: {:#04x} Ticks: {}", 
            self.registers.a, self.registers.f, self.registers.b, self.registers.c, self.registers.d, self.registers.e, self.registers.h, self.registers.l, self.memory.borrow().sp, self.memory.borrow().pc, self.ticks)
    }
...
impl CPU {
    ...
    fn handle_interrupts(&mut self) {
        ...
        let interrupt_flags = self.memory.borrow().read_byte(0xff0f);
        ...
        self.memory.borrow_mut().write_byte(0xff0f, interrupt_flags & !interrupt);
        ...
        match instruction.itype {
            ...
            InstructionType::RET(condition) => {
                let should_jump = match condition {
                    ...
                    JumpCondition::C => self.registers.get_flag(Flag::Carry),
                    _ => {
                        println!("Invalid condition for RET: {:?}", condition);
                        false
                    },
                };
                if should_jump {
